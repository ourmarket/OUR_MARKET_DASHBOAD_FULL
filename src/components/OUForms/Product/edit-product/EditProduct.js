...
import { usePutProductMutation } from "api/productApi"; // Cambiado a Put
...
/**
 * @param {Object} productData - El objeto del producto a editar
...
function EditProductForm({ productData, listCategories }) {
  ...
  const [updateProduct, { isLoading }] = usePutProductMutation();
  const [createCategory] = usePostCategoryMutation();

  // Sincronizar urlImage si productData cambia externamente
  useEffect(() => {
    if (productData?.img) {
      setUrlImage(productData.img);
    }
  }, [productData]);
  ...
  const formik = useFormik({
    initialValues: {
      ...
      offerFrom: productData?.offerFrom
        ? new Date(productData.offerFrom).toISOString().split("T")[0]
        : "",
      offerTo: productData?.offerTo
        ? new Date(productData.offerTo).toISOString().split("T")[0]
        : "",
  ...
  return (
    <MDBox p={3} component="form" onSubmit={formik.handleSubmit}>
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} lg={8}>
          <Stack spacing={3}>
            ...
            <Paper
              elevation={0}
              sx={{ p: 3, border: "1px solid #d3d3d3", borderRadius: "12px" }}
            >
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={6}>
                  <TextField
                    ...
                    label="¿Es una oferta?"
                    name="hasOffer"
                    value={formik.values.hasOffer.toString()}
                    onChange={(e) =>
            ...
            >
              ...
              <Box sx={{ p: 3 }}>
                ...
                <Alert severity="info" sx={{ borderRadius: "12px", mt: 2 }}>
                  Asegúrate de que la imagen sea clara y tenga un fondo
                  neutro...
                </Alert>
              </Box>
            </Paper>
      ...
      {/* MODAL CATEGORÍA (Se mantiene igual) */}
      <Dialog
