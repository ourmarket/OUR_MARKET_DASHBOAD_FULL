...
function CustomMarker({ lat, lng, data }) {
        ...
        <InfoWindow position={{ lat, lng }} onCloseClick={handleClose}>
          <div style={divStyle}>
            ...
            {data?.horarioApertura && (
              <p>
                Horario de {data.horarioApertura}hs a {data.horarioCierre}hs
              </p>
            )}

            <Link to={`/negocios/detalle/${data._id}`}>Ver Negocio</Link>
          </div>
        </InfoWindow>
      )}
    </Marker>
...
function MapsNegocios({ clientAddress, zones }) {
